...
MODIFY_REG(RCC->CFGR, RCC_CFGR_SW, RCC_CFGR_SW_PLL);                               //Выбираем PLLв качестве System Clock
...
//Предделитель на выходе MCO2 (P9) = 5
...
void SysTick_Init(void)
{
    ...
    MODIFY_REG(SysTick->VAL, SysTick_VAL_CURRENT_Msk,0 << SysTick_VAL_CURRENT_Pos);       //Начнём считать со значения 0
